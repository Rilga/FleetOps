<nav x-data="{ open: false }" class="bg-[#111827] text-gray-400 w-full lg:w-64 lg:fixed lg:inset-y-0 lg:left-0 flex flex-col border-r border-gray-800 z-50">
    ...
    <div class="flex-grow px-4 space-y-2 overflow-y-auto">
        <a href="{{ route('user.dashboard') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 hover:text-white' }}">
            <i class="fas fa-th-large text-lg"></i>
            <span class="font-medium">Dashboard</span>
        </a>

        <a href="{{ route('user.fleet') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.fleet') ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 hover:text-white' }}">
            <i class="fas fa-ship text-lg"></i>
            <span class="font-medium">Fleet</span>
        </a>
        <a href="{{ route('user.alerts') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('user.alerts') ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 hover:text-white' }}">
            <i class="fas fa-exclamation-triangle text-lg"></i>
            <span class="font-medium">Alert Center</span>
        </a>
    </div>
    ...
    <div class="p-4 border-t border-gray-800">
        <div class="flex items-center gap-3 px-2 py-3 mb-2">
            ...
            <div class="flex flex-col overflow-hidden">
                ...
                <span class="text-[10px] text-gray-500 font-bold uppercase">Engineer</span>
            </div>
        </div>
