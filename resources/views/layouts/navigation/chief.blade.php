<nav x-data="{ open: false }" class="bg-[#111827] text-gray-400 w-full lg:w-64 lg:fixed lg:inset-y-0 lg:left-0 flex flex-col border-r border-gray-800 z-50">
    <div class="p-6">
        <a href="{{ route('chief.dashboard') }}" class="flex items-center gap-3">
    ...
    <div class="flex-grow px-4 space-y-2 overflow-y-auto">
        <a href="{{ route('chief.dashboard') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('chief.dashboard') ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 hover:text-white' }}">
            <i class="fas fa-th-large text-lg"></i>
            <span class="font-medium">Dashboard</span>
        </a>
        <a href="{{ route('chief.approvals') }}" 
           class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-200 group {{ request()->routeIs('chief.approvals') ? 'bg-blue-600 text-white' : 'hover:bg-gray-800 hover:text-white' }}">
